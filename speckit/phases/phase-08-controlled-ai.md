...
3. **Source-grounded summarization**: case and document summaries where every
   sentence carries claim/source citations; uncited sentences are a rendering
...
- P3: SDK + ontology grounding for prompts/schemas.
- P4: workspace (assistant and citation UX); P2 review queue UI (reused).
- P6 (soft): findings/caveat plumbing reused for AI-origin suggestions.

## Exit criteria

- [ ] Every AI output type (extraction, translation-claim, summary, hypothesis
      suggestion, contradiction candidate) lands as a suggestion or derivative
      with source references — a test proves **zero direct writes** to
      canonical tables from any AI code path.
- [ ] Assistant answers render with claim-ID citations; an answer with an
      uncited assertion fails its test.
- [ ] Extraction v2 beats the Phase-1 Gemini pass on the labeled sample
      (precision on typed predicates), measured and recorded.
- [ ] A translation derivative carries parent hash, tool, version, parameters;
      deleting derivatives and re-running reproduces them.
- [ ] Promotion of any model/prompt config to production has a recorded eval
      result.
