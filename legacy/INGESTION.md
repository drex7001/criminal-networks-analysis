...
> **⚠️ UNSAFE FOR GOVERNED DATA — legacy prototype only.** These commands write
> mutable files under `data/real/` and feed the pre-Aegis graph/LLM pipeline.
> Do not use them for new governed sources. Use the active
> [`docs/INGESTION.md`](../docs/INGESTION.md) workflow (`aegis ingest ...`),
> which records provenance in the vault and sends extraction output to review.
