...
| Article VI — authorization at query time | `tests/component/test_auth.py` (incl. clock-skew leeway bounds), `tests/component/test_route_gating.py` (no ungated route, no `public_route` symbol, only the workspace bundle is mounted), `tests/component/test_workspace_serving.py` (API prefixes never fall back to the SPA), `tests/integration/test_api.py`, `tests/integration/test_authz.py`, `tests/integration/test_graph_routes.py`, `tests/system/test_authz_openfga.py` | Implemented, and since T22 with **no exemption**: the anonymous `/api/*` surface and the `public_route` lint branch are deleted (ADR-026). Phase 2 field matrix planned in T24a/T24b |
...
| Phase 2 provenance panel/contradictions | `tests/integration/test_why_connected.py` (claims, three separate grading dimensions, corroboration beside contradiction, mention anchors, identity-decision line, clearance filtering applied in the query, 404-not-403) | API implemented in T21; the browser panel that renders it is T23c |
