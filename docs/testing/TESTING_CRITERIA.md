...
| Article VII — machines suggest, humans decide | `tests/integration/test_ingestion.py`, `tests/integration/test_actions.py`, `tests/integration/test_review_envelope.py`, `tests/unit/test_suggestion_idempotency.py`, `tests/integration/test_mvp_fixture.py` (cached and structural producers leave canonical counts unchanged until the human-actored smoke acceptance) | Implemented: the typed envelope ships in T17, so acceptance dispatches per kind and every kind is either acceptable or explicitly refused by name; identity-candidate dispatch lands with `adjudicate_identity` in T20, UI in T23b; T25 pins the same invariant across the offline demo loader |
| Article VIII — disagreement/retraction preserved | `tests/integration/test_actions.py`, `tests/integration/test_api.py`, `tests/integration/test_entity_provenance.py`, `tests/integration/test_mvp_fixture.py` | Implemented: claim retraction and contradiction display are shipped; T25's corpus carries two source-attributed DOBs linked explicitly as `contradicts` |
...
| Phase 2 transliteration quality thresholds | `tests/unit/test_mention_normalize.py` (blocking keys preserve Sinhala/Tamil, stay slugify-compatible on Latin), `tests/unit/test_translit.py` (Latin/script/phonetic keys), `tests/integration/test_splink_pipeline.py` (seeded Sinhala/English pair scores above threshold with its waterfall persisted; the hard negative is not proposed), `tests/integration/test_mention_extraction.py` | Implemented T17/T19 and now measured by the T26 golden-set gate below; the live emission threshold remains pinned in `aegis/er/settings.py` |
| Phase 2 ER numeric quality gates | `tests/unit/test_er_evaluation.py` (production matcher/model boundary, exact first-run precision/recall/review-load, CLI artifact, below-gate failure, truth-label validation, fictional identifier guard), `tests/integration/test_er_evaluation.py` (full T25 pipeline writes zero identity decisions and keeps seeded Ruwan namesakes in distinct active entities), `make test-er-evaluation` | Implemented T26: `rules-v1` precision **1.000**, seeded variant recall **1.000**, review load **33.33/1,000**; CI fails against the spec 05 §6 floors and uploads the digest/settings-stamped JSON report even on failure |
...
| Phase 2 fictional MVP fixture/headless loop | `tests/integration/test_mvp_fixture.py` (7 cases: one-command load, complete fixture features, idempotent replay, cached-semantic provenance + prompt-drift refusal, machine actor refusal, structural suggestion accepted by a named reviewer then projected, fixture-only reset + mixed-state refusal), `ui/e2e/provenance.spec.ts` (admin-only rebuild + graph refresh), `tests/contract/test_mvp_demo_runbook.py` (operator labels/order, fixture identity and governance observations, disposable cleanup, exact Keycloak login/logout origins, manual no-egress real-OSINT boundary), explicit `make test-mvp` CI step, and manual `MAN-P2-001` | Implemented T25/T27 with no hosted-model dependency. `docs/MVP_DEMO.md` is the blocking UI operator path; its authorized real-OSINT appendix is manual and non-blocking |
