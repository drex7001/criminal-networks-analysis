...
| Phase 2 reversible identity/stale revision | `tests/integration/test_identity_ledger.py` (baseline revision, one-active-membership index, close-and-reopen, anchor coherence), `tests/contract/test_schema.py` (invariant is a partial unique index, not service code), `tests/integration/test_schema.py` (0007 up/down clean) | Ledger schema and baseline implemented in T17; the seven reversal cases in `speckit/specs/05-entity-resolution.md` §8 need the adjudication actions — planned T20, with projection reversal in T21 |
| Phase 2 provenance panel/contradictions | Future component + browser E2E suites | Planned: T23 |
| Phase 2 transliteration quality thresholds | `tests/unit/test_mention_normalize.py` (blocking keys preserve Sinhala/Tamil and stay slugify-compatible on Latin), `tests/integration/test_mention_extraction.py` (script detection, offsets only when verifiable); future ER evaluation suite for the numeric floors in `speckit/specs/05-entity-resolution.md` §6 | Blocking keys and script detection implemented in T17; scoring in T19, eval harness T26 |
